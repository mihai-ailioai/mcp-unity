...
export function registerGetImportSettingsTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to read import settings');
    }
