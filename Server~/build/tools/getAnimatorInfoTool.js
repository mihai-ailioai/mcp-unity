...
const toolDescription = "Inspects an Animator on a GameObject and returns controller, parameter, layer, state, and transition details. " +
...
const paramsSchema = z.object({
    ...
    includeClipDetails: z
        .boolean()
        .optional()
        .describe("When true, includes clip metadata (length, frameRate, loop, curve bindings with full keyframe data [time, value, inTangent, outTangent], and events) for clip-backed states and BlendTree children. Essential for reading existing animation values before modifying clips."),
});
export function registerGetAnimatorInfoTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, rawParams) {
    const parsedParams = paramsSchema.safeParse(rawParams);
    if (!parsedParams.success) {
        throw new McpUnityError(ErrorType.VALIDATION, `Invalid parameters for ${toolName}`, parsedParams.error.flatten());
    }
