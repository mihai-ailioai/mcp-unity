...
export function registerUpdateImportSettingsTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if ((!settings || Object.keys(settings).length === 0) &&
        ...
        throw new McpUnityError(ErrorType.VALIDATION, "At least one of 'settings' or 'platformOverrides' must be provided and non-empty");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to update import settings');
    }
