...
const paramsSchema = z.object({
    paths: z.array(z.string()).optional().describe('Optional list of asset paths to incrementally re-index (e.g. ["Assets/Scripts/Player.cs", "Assets/Prefabs/Player.prefab"]). ' +
...
async function incrementalIndexHandler(mcpUnity, contextEngine, filePaths, extra, logger) {
    const unityProjectRoot = process.cwd();
    // Split paths by type
    const scriptPaths = [];
    const prefabScenePaths = [];
    ...
    if (prefabScenePaths.length > 0) {
        try {
            const response = (await mcpUnity.sendRequest({ method: 'collect_project_assets', params: { assetPaths: prefabScenePaths } }, { timeout: 120000 }));
    ...
    const details = [];
    if (batchStats.newlyUploaded > 0)
        details.push(`${batchStats.newlyUploaded} updated`);
    if (batchStats.alreadyUploaded > 0)
        details.push(`${batchStats.alreadyUploaded} unchanged`);
    if (batchStats.skipped > 0)
        details.push(`${batchStats.skipped} skipped (oversized)`);
    if (details.length > 0)
