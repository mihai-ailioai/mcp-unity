...
export function registerControlEditorTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params, logger) {
    ...
    if (triggersReload) {
        ...
        if (noOpResponse) {
            return {
                content: [
                    { type: 'text', text: noOpResponse.message || 'No state change needed.' },
                    { type: 'text', text: JSON.stringify(noOpResponse.editorState, null, 2) }
                ],
        ...
        return {
            content: [
                { type: 'text', text: `Editor ${action} command sent, but timed out waiting for Unity to come back. Unity may still be loading.` }
            ],
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to control editor state');
    }
    return {
        content: [
            { type: 'text', text: response.message || 'Editor state updated.' },
            { type: 'text', text: JSON.stringify(response.editorState, null, 2) }
        ],
