...
export function registerSetupAnimationClipTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, rawParams) {
    ...
    if (params.frameRate !== undefined && params.frameRate <= 0) {
        throw new McpUnityError(ErrorType.VALIDATION, "frameRate must be greater than 0 when provided");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || "Failed to setup animation clip");
    }
    return {
        content: [
            {
