...
const paramsSchema = z.object({
    assetPath: z
        .string()
        .describe("The asset path of the prefab (e.g., 'Assets/Prefabs/Player.prefab'). Must start with 'Assets/' and end with '.prefab'."),
    summary: z
        .boolean()
        .optional()
        .describe("When true, returns a lightweight response with only names, instanceIds, and component type names (no property details). Useful for scanning large prefab hierarchies."),
    rootPath: z
        .string()
        .optional()
        .describe("Path within the prefab hierarchy to use as the starting point (e.g., 'CarBody/Exterior'). Only the subtree at this path is included in the response."),
    namePattern: z
        .string()
        .optional()
        .describe("Filter GameObjects by name using wildcard patterns (e.g., '*Livery*', 'Wheel*'). " +
        "When set, returns a flat list of matching GameObjects instead of the full hierarchy tree. Case-insensitive."),
    componentType: z
        .string()
        .optional()
        .describe("Filter GameObjects that have this component type (e.g., 'MeshRenderer', 'BoxCollider'). " +
 ...
 */
export function registerGetPrefabInfoTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (rootPath)
        requestParams.rootPath = rootPath;
    if (namePattern)
        requestParams.namePattern = namePattern;
    if (componentType)
        requestParams.componentType = componentType;
    const response = await mcpUnity.sendRequest({
        method: toolName,
        params: requestParams,
    });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || "Failed to get prefab info from Unity");
    }
    return {
        content: [
            {
