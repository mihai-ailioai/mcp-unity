...
export function registerSetupAnimatorControllerTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, rawParams) {
    ...
    if (!params.assetPath.trim()) {
        throw new McpUnityError(ErrorType.VALIDATION, "assetPath must be a non-empty string");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || "Failed to setup animator controller");
    }
    return {
        content: [
            {
