...
export function registerCreatePrimitiveTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to create primitive');
    }
    return {
        content: [{
                type: response.type,
