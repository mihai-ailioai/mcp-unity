...
const toolDescription = 'Finds GameObjects in the current scene hierarchy using one or more filters (componentType, namePattern, tag, layer). ' +
...
async function toolHandler(mcpUnity, rawParams) {
    ...
    const hasFilter = (params.componentType?.trim().length ?? 0) > 0 ||
        (params.namePattern?.trim().length ?? 0) > 0 ||
        (params.tag?.trim().length ?? 0) > 0 ||
        params.layer !== undefined;
    if (!hasFilter) {
        throw new McpUnityError(ErrorType.VALIDATION, 'At least one filter must be provided: componentType, namePattern, tag, or layer');
    }
