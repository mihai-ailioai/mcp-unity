...
export function registerUpdateScriptableObjectTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!params.fieldData || Object.keys(params.fieldData).length === 0) {
        throw new McpUnityError(ErrorType.VALIDATION, "Required parameter 'fieldData' must be provided and non-empty");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to update ScriptableObject');
    }
