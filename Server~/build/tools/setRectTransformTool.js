...
export function registerSetRectTransformTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!hasLayoutInput) {
        throw new McpUnityError(ErrorType.VALIDATION, "At least one of 'preset', 'anchoredPosition', 'sizeDelta', 'anchorMin', 'anchorMax', 'pivot', 'offsetMin', or 'offsetMax' must be provided");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to set RectTransform');
    }
