...
const operationSchema = z.object({
    ...
    id: z
        .string()
        .optional()
        .describe("Optional identifier for this operation (for tracking in results)"),
});
const paramsSchema = z.object({
    assetPath: z
        .string()
        .describe("The asset path of the prefab to modify (e.g., 'Assets/Prefabs/Player.prefab'). Must start with 'Assets/' and end with '.prefab'."),
    variantPath: z
        .string()
        .optional()
        .describe("If provided, creates a prefab variant from assetPath at this path before applying operations. Must start with 'Assets/' and end with '.prefab'. When set, 'operations' becomes optional."),
    operations: z
        .array(operationSchema)
        .max(100, "Maximum of 100 operations allowed")
        .optional()
        .describe("Array of operations to execute against the prefab contents. Same format as batch_execute operations. Required unless 'variantPath' is specified."),
    stopOnError: z
        .boolean()
        .optional()
        .default(true)
 ...
 */
export function registerModifyPrefabTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params, logger) {
    const { assetPath, variantPath, operations, stopOnError } = params;
    // Validate assetPath
    if (!assetPath || assetPath.trim().length === 0) {
        throw new McpUnityError(ErrorType.VALIDATION, "assetPath is required and cannot be empty");
    }
    if (!assetPath.startsWith("Assets/")) {
        throw new McpUnityError(ErrorType.VALIDATION, "assetPath must start with 'Assets/'");
    }
    if (!assetPath.endsWith(".prefab")) {
        throw new McpUnityError(ErrorType.VALIDATION, "assetPath must end with '.prefab'");
    }
    // Validate variantPath if provided
    if (variantPath) {
        if (!variantPath.startsWith("Assets/")) {
            throw new McpUnityError(ErrorType.VALIDATION, "variantPath must start with 'Assets/'");
        }
        if (!variantPath.endsWith(".prefab")) {
            throw new McpUnityError(ErrorType.VALIDATION, "variantPath must end with '.prefab'");
        }
    }
    // Validate operations — required unless creating a variant
    if (!variantPath && (!operations || operations.length === 0)) {
        throw new McpUnityError(ErrorType.VALIDATION, "The 'operations' array is required and must contain at least one operation (unless 'variantPath' is specified)");
    }
    ...
    if (operations) {
        for (const op of operations) {
            if (blockedTools.has(op.tool)) {
                throw new McpUnityError(ErrorType.VALIDATION, `Tool '${op.tool}' is not allowed inside modify_prefab (it affects scenes or project state outside the prefab editing context)`);
            }
            if (!op.tool || op.tool.trim().length === 0) {
                throw new McpUnityError(ErrorType.VALIDATION, "Each operation must have a non-empty 'tool' name");
            }
        }
    }
    logger.info(`Sending modify_prefab with ${operations?.length ?? 0} operations for ${assetPath}${variantPath ? ` (variant: ${variantPath})` : ""} to Unity`);
    const requestParams = {
        assetPath: assetPath,
    ...
    if (response.results && operations) {
        for (let i = 0; i < response.results.length; i++) {
            ...
            if (opResult.success && opResult.result && op && readOnlyTools.has(op.tool)) {
                content.push({
                    type: "text",
                    text: JSON.stringify({ operationId: opResult.id, tool: op.tool, result: opResult.result }, null, 2),
                });
            }
        }
    }
