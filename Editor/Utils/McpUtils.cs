...
function findServer() {{
  // 1. Library/PackageCache (git/registry installs — folder includes version hash)
  const cache = ""Library/PackageCache"";
  if (existsSync(cache)) {{
    const entry = readdirSync(cache).find(d => d.startsWith(""{packageName}""));
    if (entry) return join(cache, entry, ""Server~"", ""build"", ""index.js"");
  }}
  // 2. Packages/ directory (local or embedded packages)
  const local = join(""Packages"", ""{packageName}"", ""Server~"", ""build"", ""index.js"");
  if (existsSync(local)) return local;
  console.error(""mcp-unity package not found. Searched Library/PackageCache and Packages/."");
  process.exit(1);
}}
