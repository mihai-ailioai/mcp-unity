...
- `modify_prefab` — Modify a prefab asset headlessly via batched operations (same format as batch_execute) using LoadPrefabContents isolated editing context. Supports `find_gameobjects` and `add_asset_to_scene` as sub-operations for searching within and nesting prefabs. **Important:** `instanceId`/`newParentId` params are rejected in sub-operations — always use `objectPath`/`newParent` path strings
